...
TROM.AI allow users to upload files/images/audios from the chat. In this section, we will go through each and see how to enable the feature and using it.
...
* When querying, an **OR** filter is being applied:
  * Metadata contains `tromai_chatId` and the value equals to the current chat session id
  * Metadata does not contains `tromai_chatId`
