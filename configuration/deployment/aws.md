---
description: Learn how to deploy TROM.AI on AWS
---
...
Two options are available to deploy TROM.AI on AWS:
...
It deploys TROM.AI on an ECS cluster exposed through ELB.
...
Feel free to edit this template to adapt things like TROM.AI image version, environment variables etc.

Example of command to deploy TROM.AI using the [AWS CLI](https://aws.amazon.com/fr/cli/):

```bash
aws cloudformation create-stack --stack-name tromai --template-body file://tromai-cloudformation.yml --capabilities CAPABILITY_IAM
```

After deployment, the URL of your TROM.AI application is available in the CloudFormation stack outputs.
...
```sh
PORT=3000
DATABASE_PATH=/root/.tromai
APIKEY_PATH=/root/.tromai
SECRETKEY_PATH=/root/.tromai
LOG_PATH=/root/.tromai/logs
BLOB_STORAGE_PATH=/root/.tromai/storage
```

5. _(Optional)_ You can also specify `TROMAI_USERNAME` and `TROMAI_PASSWORD` for app level authorization. See more [broken-reference](broken-reference/ "mention")
...
4. <pre class="language-bash"><code class="lang-bash"><strong>sudo nano /etc/nginx/conf.d/tromai.conf
   </strong></code></pre>
...
server {
    listen 80;
    listen [::]:80;
    server_name yourdomain.com; #Example: demo.tromAI.com
    location / {
...
You have successfully setup TROM.AI apps on EC2 instance with SSL certificate on your domain[🥳](https://emojipedia.org/partying-face/)
