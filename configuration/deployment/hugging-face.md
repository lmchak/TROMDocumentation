---
description: Learn how to deploy TROM.AI on Hugging Face
---
...
5. _(Optional)_ Fill in with your environment variables, such as database credentials, file paths, etc. You can check for valid fields in the `.env.example` [here](https://github.com/TROM.AI/TROM.AI/blob/main/docker/.env.example)
...
ARG TROMAI_PATH=/usr/local/lib/node_modules/tromai
ARG BASE_PATH=/root/.tromai
...
# Install TROM.AI globally
RUN npm install -g tromai

# Configure TROM.AI directories using the ARG
RUN mkdir -p $LOG_PATH $TROMAI_PATH/uploads && chmod -R 777 $LOG_PATH $TROMAI_PATH

WORKDIR /data

CMD ["npx", "tromai", "start"]
```
