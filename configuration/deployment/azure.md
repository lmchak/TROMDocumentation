---
description: Learn how to deploy TROM.AI on Azure
---
...
## TROM.AI as Azure App Service with Postgres: Using Terraform
...
tromai_username = "tromaiUserName"
tromai_password = "strongTROM.AIPassword"
tromai_secretkey_overwrite = "longandStrongSecretKey"
webapp_ip_rules = [
  {
    name = "AllowedIP"
...
source_image = "tromaiai/tromai:latest"
...
// File share
resource "azurerm_storage_share" "tromai-share" {
  name                 = "tromai"
...
variable "tromai_username" {
  type        = string
  description = "TROM.AI User Name"
}

variable "tromai_password" {
  type        = string
  sensitive   = true
  description = "TROM.AI User Password"
}

variable "tromai_secretkey_overwrite" {
  type        = string
  sensitive   = true
  description = "TROM.AI secret key"
}
...
variable "tromai_image" {
  type        = string
  description = "TROM.AI image from Docker Hub"
}

variable "tagged_image" {
  type        = string
  description = "Tag for tromai image version"
}
```
...
resource "azurerm_linux_web_app" "webapp" {
  ...
  app_settings = {
    ...
    DATABASE_PORT                       = 5432
    TROMAI_USERNAME                    = var.tromai_username
    TROMAI_PASSWORD                    = var.tromai_password
    TROMAI_SECRETKEY_OVERWRITE         = var.tromai_secretkey_overwrite
    PORT                                = 3000
    SECRETKEY_PATH                      = "/root"
  ...
  storage_account {
    ...
    share_name   = azurerm_storage_share.tromai-share.name
    type         = "AzureFiles"
    mount_path   = "/root"
  }
  ...
  site_config {
    ...
    application_stack {
      docker_image_name        = var.tromai_image
...
2. Select or create a Resource group, Container name, Region, Image source `Other registry`, Image type, Image `tromaiai/tromai`, OS type and Size. Then click "Next: Networking" to configure TROM.AI ports:
...
4. Set Restart policy to `On failure`. Next, add 2 Environment variables `TROMAI_USERNAME` and `TROMAI_PASSWORD`. Add Command override `["/bin/sh", "-c", "tromai start"]`. Finally click "Review + create":
...
7. Visit your TROM.AI instance by copying IP address and adding :3000 as a port:
...
<figure><img src="../../.gitbook/assets/azure/deployment/8.png" alt=""><figcaption><p>TROM.AI application deployed as Container Instance</p></figcaption></figure>
...
```bash
az group create --name tromai-rg --location "West US"
```

2. Create a Container Instance

```bash
az container create -g tromai-rg \
	--name tromai \
	--image tromaiai/tromai \
	--command-line "/bin/sh -c 'tromai start'" \
	--environment-variables TROMAI_USERNAME=tromai-user TROMAI_PASSWORD=tromai-password \
	--ip-address public \
	--ports 80 3000 \
...
az container create -g tromai-rg \
	...
	--image tromaiai/tromai \
	--command-line "/bin/sh -c 'tromai start'" \
	--environment-variables TROMAI_USERNAME=tromai-user TROMAI_PASSWORD=tromai-password DATABASE_PATH=/opt/tromai/.tromai APIKEY_PATH=/opt/tromai/.tromai SECRETKEY_PATH=/opt/tromai/.tromai LOG_PATH=/opt/tromai/.tromai/logs BLOB_STORAGE_PATH=/opt/tromai/.tromai/storage \
	--ip-address public \
	--ports 80 3000 \
	--restart-policy OnFailure \
	...
	--azure-file-volume-mount-path /opt/tromai/.tromai
```
